...
pg.init()
s_s = (400, 300)
screen = pg.display.set_mode(s_s, pg.RESIZABLE)
pg.display.set_caption("Physics Simulation")
run = True

# classes
class ball: # for all ball objects
  def __init__ (self, pos, vel, r, fmass, rot, rotv, fric, color):
...
def draw_half_plane(display, color, angle, position):
  ...
  for i in range(4):
    ...
    side_p1 = (p1 - position).dot(normal_vector)
    side_p2 = (p2 - position).dot(normal_vector)
...
precision = 20 # ticks per second. does not influence gametime. (turning this up will make it laggier, though.)
